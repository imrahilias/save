...
program sadoku
  ...
  cool : do while ( t .gt. tf )
     ...
     markov : do n = 1,chains
        ...
        de = sum(abs(coltemp(:,:))) + sum(abs(rowtemp(:,:))) - sum(abs(col(:,:))) - sum(abs(row(:,:)))
...
contains
  ...
  subroutine printoku(text)
    ...
    write(*,*) 'run: ',c, ', temperatur:',t, ', flips: ',flips, ', energy: ',energy
    write(*,*) 'chains: ',chains, ', runs: ',c*chains+n, ', reheats:', reheat, ', accepts: ',accept
